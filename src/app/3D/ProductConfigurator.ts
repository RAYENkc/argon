...
export class ProductConfigurator {
  ...
  public loadInitialItem(id : any): void {
    const snapshot = this.activatedRouter.snapshot;
    const name = snapshot.paramMap.has("name") ? snapshot.paramMap.get("name")!.toLowerCase() : "";
    const selectedItem = this.productConfiguratorService.items.find(i => i.name.toLowerCase() === name) || this.productConfiguratorService.items[id];
